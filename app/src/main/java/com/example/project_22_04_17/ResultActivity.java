package com.example.project_22_04_17;
...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < voteResult.length; ++i) {
            if (voteResult[maxldx] < voteResult[i])
                maxldx = i;
        }
        TextView BestTv = (TextView) findViewById(R.id.BestTv);
        ImageView BestIv = (ImageView) findViewById(R.id.BestIv);

        BestTv.setText(imageName[maxldx]);
        BestIv.setImageResource(imageField[maxldx]);
        ...
        // 각 TextView 및 RationgBar에 넘겨 받은 값을 반영.
        for (int i = 0; i < voteResult.length; i++) {
